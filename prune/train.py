...
from model import GPTConfig, GPT, L1UnstructuredPruneableGPT, L2StructuredPruneableGPT
# from prune.model import L1UnstructuredPruneableGPT, L2StructuredPruneableGPT
# from model2 import L2PruneableGPT
...
if pruneable:
    ...
    pretrained_model_cls = L1UnstructuredPruneableGPT if prune_l1 else L2StructuredPruneableGPT
...
if loss_guided_prune:
    ...
    orig_num_params = model.num_elems
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                ckpt_path = os.path.join(out_dir, 'ckpt.pt')
                if pruneable:
                    fp, metadata = model.pruning_metadata
                    ckpt_path = os.path.join(out_dir, fp)
    ...
    if iter_num > max_iters:
        if not pruneable or model.num_pruned == prune_max_iters:
            break

        if loss_guided_prune:
            i = 0
            while True:
                num = model.num_channels if prune_l2 else model.num_elems
                denom = model._orig_num_channels if prune_l2 else orig_num_params
                pct_original = num / denom
                if math.isclose(pct_original, 0.1, abs_tol=0.05) or pct_original < 0.1: 
